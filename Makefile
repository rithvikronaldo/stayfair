...
DB_URL ?= postgres://postgres:postgres@localhost:5433/acta?sslmode=disable
...
psql:
	docker exec -it acta-postgres psql -U postgres -d acta

seed:
	docker exec -i acta-postgres psql -U postgres -d acta < testdata/seed.sql
